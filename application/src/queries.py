...
def hourly_average(room, day):
	...
	c = conn.cursor()
	c.execute('SELECT round(avg(assoc_devices)) FROM wifi_db.wifi_log WHERE room_id = 2 AND FROM_UNIXTIME(event_time,"%w") = 1 AND FROM_UNIXTIME(event_time,"%H") >= 08 GROUP BY FROM_UNIXTIME(event_time,"%H")')
	data = c.fetchall()
	for i in data:
